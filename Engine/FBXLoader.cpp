...
void FBXLoader::Import(const wstring& path)
{
	...
	string strPath = WstringToString(path);
...
void FBXLoader::LoadMesh(FbxMesh* mesh)
{
	...
	meshInfo.name = StringToWstring(mesh->GetName());
...
void FBXLoader::LoadMaterial(FbxSurfaceMaterial* surfaceMaterial)
{
	...
	material.name = StringToWstring(surfaceMaterial->GetName());
...
void FBXLoader::CreateTextures()
{
	...
	//				GET_SINGLE(Resources)->Get<Texture>(WstringToString(filename));
	//		}
	...
	for (size_t i = 0; i < _meshes.size(); i++)
	{
		for (size_t j = 0; j < _meshes[i].materials.size(); j++)
		{
...

			{
				...
				if (diffuseTexture)
					material->SetDiffuseSrvHeapIndex(diffuseTexture->GetTexHeapIndex());
			}

			{
				...
				if (normalTexture)
					material->SetNormalSrvHeapIndex(normalTexture->GetTexHeapIndex());
			}

			{
				...
				if (specularTexture)
					material->SetRoughnessSrvHeapIndex(specularTexture->GetTexHeapIndex());
			}
...
void FBXLoader::LoadBones(FbxNode* node, int32 idx, int32 parentIdx)
{
	...
	if (attribute && attribute->GetAttributeType() == FbxNodeAttribute::eSkeleton)
	{
		...
		bone->boneName = StringToWstring(node->GetName());
...
void FBXLoader::LoadAnimationInfo()
{
	...
	for (int32 i = 0; i < animCount; i++)
	{
		...
		animClip->name = StringToWstring(animStack->GetName());
